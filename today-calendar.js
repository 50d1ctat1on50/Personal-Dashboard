...
async function loadDailyAgentSuggestion(date) {
  ...
  try {
    ...
    if (!response.ok) {
  suggestionsEl.innerHTML = `
    <strong>Plan for the Day:</strong><br><br>
    AI plan could not load.<br>
    <small>
      Status: ${response.status}<br>
      Error: ${data.error || 'Unknown error'}<br>
      Details: ${data.details || 'No details returned'}<br>
      Model: ${data.modelTried || 'Not available'}
    </small>
  `;
  return;
}
...
suggestionsEl.innerHTML = `
  ...
  ${data.modelUsed ? `<br><br><small>Generated by ${data.modelUsed}</small>` : ''}
`;
  } catch (error) {
  suggestionsEl.innerHTML = `
    <strong>Plan for the Day:</strong><br><br>
    AI plan could not load due to a browser/script error.<br>
